...
uv run python index_fandom.py --resume                  # продолжить с чекпоинта, не с начала списка
```
...
**`--resume`** — только для прогона по всему корпусу (без `--title`/`--limit`).
Без него список статей каждый раз запрашивается у Fandom заново и перебирается
с первой буквы алфавита — уже залитые просто пропускаются по `revid`, это не
тратит квоту, но тратит время. `--resume` читает `index_checkpoint.json`
(заголовок статьи, на которой остановился прошлый прогон, пишется на каждую
статью) и продолжает ровно с неё через параметр MediaWiki `apfrom` — не с
начала списка и не с границы очередной страницы в 500 статей, а с точного
места. Файл удаляется сам, когда корпус пройден целиком без обрыва по квоте.
